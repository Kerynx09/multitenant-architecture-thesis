...
def request_connection(thread_id, engine, results):
    """
    Make a single threaded request that holds a connection.
    In the fixed version, the pool is large enough to support all threads.
    """
    try:
...
def simulate_pool_management(tenant_name):
    ...
    print(f'Running fixed connection simulation for tenant `{tenant_name}`')
    results = []
    threads = []
...
if __name__ == '__main__':
    simulate_pool_management('school_oakwood')
