"""
Issue 3 (Before): Sequential schema migration on test table.
Uses a separate test table 'migration_test' to avoid touching existing data.
"""
...
log_filename = f"{log_dir}/migration_before_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def add_column(engine, tenant_name):
    """Add a new column to the test table (simulating a migration)."""
    with engine.connect() as conn:
...
def main():
    logger.info("=== Starting SEQUENTIAL migration (before) ===")
    results = []
    total_start = time.time()

    for tenant in TENANTS:
        elapsed, status = migrate_tenant(tenant)
        results.append((tenant, elapsed, status))
    ...
    print(f"Average per tenant: {total_elapsed/len(TENANTS):.3f} seconds")
    for tenant, elapsed, status in results:
